...
import cn.origin.cube.module.modules.visual.ItemPhysics;
        ...
        if (ItemPhysics.INSTANCE.isEnabled()) {
                ...
                if (entity.onGround) {
                    if (entity.rotationPitch != 0.0f && entity.rotationPitch != 90.0f && entity.rotationPitch != 180.0f && entity.rotationPitch != 270.0f) {
                        ...
                        if (d0 <= d2 && d0 <= d3 && d0 <= d4) {
                            float f = entity.rotationPitch = entity.rotationPitch < 0.0f ? (entity.rotationPitch = entity.rotationPitch + (float)rotation) : (entity.rotationPitch = entity.rotationPitch - (float)rotation);
                        }
                        if (d2 < d0 && d2 <= d3 && d2 <= d4) {
                            float f = entity.rotationPitch = entity.rotationPitch - 90.0f < 0.0f ? (entity.rotationPitch = entity.rotationPitch + (float)rotation) : (entity.rotationPitch = entity.rotationPitch - (float)rotation);
                        }
                        if (d3 < d2 && d3 < d0 && d3 <= d4) {
                            float f = entity.rotationPitch = entity.rotationPitch - 180.0f < 0.0f ? (entity.rotationPitch = entity.rotationPitch + (float)rotation) : (entity.rotationPitch = entity.rotationPitch - (float)rotation);
                        }
                        if (d4 < d2 && d4 < d3 && d4 < d0) {
                            entity.rotationPitch = entity.rotationPitch - 270.0f < 0.0f ? (entity.rotationPitch = entity.rotationPitch + (float)rotation) : (entity.rotationPitch = entity.rotationPitch - (float)rotation);
                        }
                    }
                } else {
                    ...
                    entity.rotationPitch = flag2 | material == Material.WATER | material2 == Material.WATER | flag3 ? (entity.rotationPitch = entity.rotationPitch + (float)(rotation / 4.0)) : (entity.rotationPitch = entity.rotationPitch + (float)(rotation * 2.0));
                }
            }
            ...
            for (int j = 0; j < i; ++j) {
                if (ibakedmodel.isAmbientOcclusion()) {
                    GlStateManager.pushMatrix();
                    GlStateManager.scale((float)ItemPhysics.INSTANCE.scale.getValue().floatValue(), (float)ItemPhysics.INSTANCE.scale.getValue().floatValue(), (float)ItemPhysics.INSTANCE.scale.getValue().floatValue());
